...
def handle_realtime_stream_cost_calculation(
    ...
    session_models = tuple(
        cast(OpenAIRealtimeStreamSessionEvents, result)["session"].get("model", None)
        for result in results
        if result["type"] == "session.created"
    )
    potential_model_names = tuple(filter(None, (custom_pricing_model, *session_models, litellm_model_name)))
...
def handle_realtime_transcription_cost_calculation(
    ...
    model_info = _get_model_info_or_none(model_name, custom_llm_provider)
    override_info = (
        _get_model_info_or_none(custom_pricing_model, custom_llm_provider)
...
def _transcription_usage_cost(
    ...
    if usage_type == "tokens":
        ...
        audio_cost = float(audio_tokens) * _transcription_rate(
            ("input_cost_per_audio_token", "input_cost_per_token"), override_info, model_info
        )
        text_cost = float(text_tokens) * _transcription_rate(
            ("input_cost_per_token",), override_info, model_info
        )
        output_cost = float(output_tokens) * _transcription_rate(
