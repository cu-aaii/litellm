...
class VertexAILivePassthroughLoggingHandler(BasePassthroughLoggingHandler):
    ...
    def _create_usage_object_from_metadata(
        ...
        Carries the per-modality breakdown, because the cost calculator prices audio and
        image tokens from ``*_tokens_details``. Reporting only text made every non-text
        modality bill as nothing: Gemini reports Live video as a stream of frames under
        the IMAGE modality, so a camera feed was free.
        ...
        prompt_by_modality = VertexAILivePassthroughLoggingHandler._tokens_by_modality(
            usage_metadata.get("promptTokensDetails") or []
        )
        candidates_by_modality = VertexAILivePassthroughLoggingHandler._tokens_by_modality(
            usage_metadata.get("candidatesTokensDetails") or []
        )

        prompt_tokens = usage_metadata.get("promptTokenCount", 0) or sum(prompt_by_modality.values())
        completion_tokens = usage_metadata.get("candidatesTokenCount", 0) or sum(candidates_by_modality.values())
    ...
    def vertex_ai_live_passthrough_handler(
        ...
        **kwargs,
        ...
        try:
            ...
            # Create Usage object for standard LiteLLM logging. Cost is left to the
            # standard calculator, which prices every modality from the token details.
            ...
            verbose_proxy_logger.debug(
                ...
                f"Model: {safe_model}, "
                f"Prompt tokens: {usage.prompt_tokens} {usage.prompt_tokens_details}, "
                f"Completion tokens: {usage.completion_tokens} {usage.completion_tokens_details}"
            )
